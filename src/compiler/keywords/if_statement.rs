...
// init a = 0 if (b == 2);
pub fn parse_keyword_if(output: &mut Output, lexer: &Lexer, mut lexer_index: usize) -> usize {
    output.js_output.push_str("if (");
    lexer_index += 2;
    lexer_index =  parse_args(output, lexer, lexer_index) + 2;
    output.js_output.push_str(") {");
    lexer_index
}
fn parse_args(output: &mut Output, lexer: &Lexer, mut lexer_index: usize) -> usize {
    ...
    while lexer_index < lexer.tokens.len() {
        ...
        match token.symbol_id {
            ...
            KEYWORD_CALL => { lexer_index = parse_keyword_call(output, lexer, lexer_index) - 1; }
            0 => { output.js_output.push_str(&token.text); },
            _ => { parse_symbols(&token, output); }
        }
